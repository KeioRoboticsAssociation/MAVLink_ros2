#include "stm32_mavlink_interface/servo_controller.hpp"
...
ServoController::ServoController(rclcpp::Node* node) : node_(node) {
    ...
    servo_cmd_sub_ = node_->create_subscription<stm32_mavlink_interface::msg::ServoCommand>(
        "servo/command", 10,
        std::bind(&ServoController::servoCommandCallback, this, std::placeholders::_1));
    
    servo_state_pub_ = node_->create_publisher<stm32_mavlink_interface::msg::ServoState>(
        "servo/states", 10);
    
    servo_config_srv_ = node_->create_service<stm32_mavlink_interface::srv::SetServoConfig>(
        "servo/set_config",
...
void ServoController::handleManualControl(const mavlink_manual_control_t& manual) {
    ...
    if (servo_states_.size() > 0) {
        servo_states_[0].target_angle_deg = (manual.x / 1000.0f) * 90.0f;
    }
    if (servo_states_.size() > 1) {
        servo_states_[1].target_angle_deg = (manual.y / 1000.0f) * 90.0f;
    }
    if (servo_states_.size() > 2) {
        servo_states_[2].target_angle_deg = (manual.z / 1000.0f) * 90.0f;
    }
    if (servo_states_.size() > 3) {
        servo_states_[3].target_angle_deg = (manual.r / 1000.0f) * 90.0f;
    }
}
...
void ServoController::handleRCChannelsOverride(const mavlink_rc_channels_override_t& rc_override) {
    ...
    for (size_t i = 0; i < 8 && i < servo_states_.size(); i++) {
        if (channels[i] != UINT16_MAX && channels[i] >= 1000 && channels[i] <= 2000) {
            servo_states_[i].pulse_us = channels[i];
            float normalized = (channels[i] - 1500.0f) / 500.0f;
            servo_states_[i].target_angle_deg = normalized * 90.0f;
        }
    }
}
...
bool ServoController::getManualControlMessage(mavlink_message_t& msg, uint8_t system_id, 
    ...
    if (servo_states_.size() > 0) {
        x = static_cast<int16_t>((servo_states_[0].target_angle_deg / 90.0f) * 1000.0f);
    }
    if (servo_states_.size() > 1) {
        y = static_cast<int16_t>((servo_states_[1].target_angle_deg / 90.0f) * 1000.0f);
    }
    if (servo_states_.size() > 2) {
        z = static_cast<int16_t>((servo_states_[2].target_angle_deg / 90.0f) * 1000.0f);
    }
    if (servo_states_.size() > 3) {
        r = static_cast<int16_t>((servo_states_[3].target_angle_deg / 90.0f) * 1000.0f);
    }
...
void ServoController::servoConfigCallback(
    const std::shared_ptr<stm32_mavlink_interface::srv::SetServoConfig::Request> request,
