#include "stm32_mavlink_uart/encoder_interface.hpp"
#include <cstring>
...
EncoderInterface::EncoderInterface(rclcpp::Node* node) : node_(node) {
    ...
    encoder_state_pub_ = node_->create_publisher<stm32_mavlink_uart::msg::EncoderState>(
        "encoder/states", 10);
    
    encoder_config_srv_ = node_->create_service<stm32_mavlink_uart::srv::SetEncoderConfig>(
        "encoder/set_config",
...
void EncoderInterface::encoderConfigCallback(
    const std::shared_ptr<stm32_mavlink_uart::srv::SetEncoderConfig::Request> request,
...
void EncoderInterface::publishEncoderStates() {
    ...
    for (size_t i = 0; i < encoder_states_.size(); i++) {
        auto msg = stm32_mavlink_uart::msg::EncoderState();
