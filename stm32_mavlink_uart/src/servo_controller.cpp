...
ServoController::ServoController(rclcpp::Node* node) : node_(node) {
    ...
    servo_cmd_sub_ = node_->create_subscription<stm32_mavlink_uart::msg::ServoCommand>(
        "servo/command", 10,
        std::bind(&ServoController::servoCommandCallback, this, std::placeholders::_1));
    
    servo_state_pub_ = node_->create_publisher<stm32_mavlink_uart::msg::ServoState>(
        "servo/states", 10);
    
    servo_config_srv_ = node_->create_service<stm32_mavlink_uart::srv::SetServoConfig>(
        "servo/set_config",
...
void ServoController::servoCommandCallback(const stm32_mavlink_uart::msg::ServoCommand::SharedPtr msg) {
    std::lock_guard<std::mutex> lock(servo_mutex_);
...
void ServoController::servoConfigCallback(
    const std::shared_ptr<stm32_mavlink_uart::srv::SetServoConfig::Request> request,
...
void ServoController::publishServoStates() {
    ...
    for (size_t i = 0; i < servo_states_.size(); i++) {
        ...
        auto msg = stm32_mavlink_uart::msg::ServoState();
